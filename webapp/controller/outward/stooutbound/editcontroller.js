sap.ui.define([
    ...
    function (Controller, MessageBox, Device, SearchField, TypeString, ColumnListItem, Label, Filter, UIComponent, FilterOperator, JSONModel,library,Token,ODataModel,Column,MessageToast) {
        ...
        return Controller.extend("gatepassapp.controller.outward.stooutbound.editcontroller", {
        ...
        // ===========================================================================
            ...
            onEditValueHelpRequested: function() {
                ...
                this.loadFragment({
                    name: "gatepassapp.view..outward.stooutbound.fragment.EditSalesValueHelpDialogFilterbar"
            ...
            onEditValueHelpOkPress: function (oEvent) {
                ...
                var aTokens = this.getView().byId("id_edit_sto_purchase_document_h").getTokens();
                    ...
                    if(OldTokens.length > 0){
                        ...
                        var oTable = this.getView().byId("id_storeceipt_table");
                        var oModel1 = this.getView().getModel("YY1_ZGE_OUTWARD_GATEPASS_CDS"); // Replace with your actual OData model name
                        var oFilters2 = [aFilters2];
                        var that = this;
                        oModel1.read("/YY1_ITEM_ZGE_OUTWARD_GATEPASS", {
                            filters: oFilters2,
                            success: function (oData) {
                                var aItems = oData.results; // The array of read items
                                var oTableModel = new sap.ui.model.json.JSONModel();
        ...
        OnStoChangeDocNoFragOpen:function(oEvent){
            if (!this.StoChangeDocNohead) {
                this.StoChangeDocNohead = sap.ui.xmlfragment(this.getView().getId("STODialogFrag"), "gatepassapp.view.outward.stooutbound.fragment.changedeletestoreceipt", this);
        ...
        OnStoGateEntryNoFragOpen:function(oEvent){
            if (!this.SoDocChange) {
                this.SoDocChange = sap.ui.xmlfragment(this.getView().getId("STOGateEntryDocHead01_dialog"), "gatepassapp.view.outward.stooutbound.fragment.stogateentrydocno", this);
