    ...
    function (Controller, TypeString, ColumnListItem, Label, SearchField, Token, Filter, FilterOperator, ODataModel, UIColumn, MColumn, Text) {
        ...
        return Controller.extend("gatepassapp.controller.gateentryclose.gateentryclose", {
            onInit: function () {
                ...
                this.HModel = new sap.ui.model.json.JSONModel({
                    Header:[]
                });
                this.getView().setModel(this.HModel, "HModel");
                ...
                if(myArray[0] === '1' || myArray[0] === '2' || myArray[0] === '3' || myArray[0] === '4'){
                    model0.read("/YY1_ZGE_INWARD_GATEPASS", {
                        ...
                        success: function (oData1, oRespons) {
                            ...
                            that.HModel = new sap.ui.model.json.JSONModel({
                                Header:GetSAP_UUID[0]
                            });
                            that.getView().setModel(that.HModel, "HModel");

                            // that.getView().byId("id_vehicle_no_col").setVisible(true)
                            // that.getView().byId("id_vehicle_in_col").setVisible(true)
                            // that.getView().byId("id_vehicle_out_col").setVisible(true)
                            // that.getView().byId("id_gross_weight_col").setVisible(true)
                            // that.getView().byId("id_tare_weight_col").setVisible(true)
                            // that.getView().byId("id_net_weight_col").setVisible(true)

                            // that.getView().byId("id_vehicle_no").setValue(GetSAP_UUID[0].VehicleNo)
                            // that.getView().byId("id_vehicle_in").setValue(GetSAP_UUID[0].SAP_CreatedDateTime)
                            // that.getView().byId("id_vehicle_out").setValue(GetSAP_UUID[0].SAP_LastChangedDateTime)
                            // that.getView().byId("id_gross_weight").setValue(GetSAP_UUID[0].GrossWeight)
                            // that.getView().byId("id_tare_weight").setValue(GetSAP_UUID[0].TareWeight)
                            // that.getView().byId("id_net_weight").setValue(GetSAP_UUID[0].NetWeight)
            ...
            OnClose:function(){
                    ...
                    let GrossWeight = this.getView().byId("id_gross_weight").getValue();
                    let TareWeight = this.getView().byId("id_tare_weight").getValue();
                    let NetWeight = this.getView().byId("id_net_weight").getValue();
